...
async function determineRequestType(userInput) {
  ...
  try {
    ...
    3. email_question - User is asking about emails, inbox, or wants information from their emails, or questions about events.
...
async function handleSendEmailRequest(userInput, context, conversationHistory) {
  ...
  try {
    ...
    3. The email should be professional and well written, and a proper length without adding any extra information not mentioned in the user's request.
    4. Sign it with the sender's name at the end of the email. ALWAYS include the sender's name.
    5. Do not include any considerations for the recipient, just write the email.
    ...
    Subject: [suitable email subject]  
    Body: 
    [should be a proper email message, ending with a personalized signature using the sender's name. dont add not mentioned information]
    ...
    const text = data?.candidates?.[0]?.content?.parts?.[0]?.text || "⚠️ Couldn't generate the email.";
  ...
  try {
    ...
    const formattedEmails = emails.map((email, index) => {
      ...
      return `Email ${index + 1}:
        ...
        Subject: ${subject}
        Date: ${date}
        Content: ${emailContent || email.snippet || "No content"}
      `;
    }).join('\n\n');
...
export async function sendEmail(token, to, subject, body) {
  // Make sure body newlines are preserved by converting them to proper email newlines
